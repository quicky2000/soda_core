...
{
  class dom_osm_full_extractor: public dom_simple_analyzer_if
  {
  public:
  ...
  void dom_osm_full_extractor::analyze(const osm_diff_analyzer_dom_if::t_dom_tree & p_tree)
  {
      ...
      for(int l_index = 0; l_index < l_nb_version ; ++l_index)
	{
	  ...
	  if(osm_api_data_types::osm_node::get_type_str() == l_node.getName())
	    ...
	    {
	      m_relations.push_back(generic_dom_utilities<osm_diff_analyzer_dom_if::t_dom_tree>::extract_info<osm_api_data_types::osm_relation>(l_node,true));
	    }
