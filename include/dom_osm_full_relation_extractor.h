...
{
  class dom_osm_full_relation_extractor: public dom_analyser_if
  {
  public:
  ...
  //----------------------------------------------------------------------------
  void dom_osm_full_relation_extractor::analyse(const t_dom_tree & p_tree)
  {
      ...
      for(int l_index = 0; l_index < l_nb_version ; ++l_index)
	{
	  ...
	  if(osm_node::get_type_str() == l_node.getName())
	    ...
	    {
	      m_result = generic_dom_utilities<t_dom_tree>::extract_info<osm_relation>(l_node,true);
	    }
	  else
	    {
