...
namespace osm_diff_watcher
{
  ...
  {				       
  public:
    module_manager(soda_Ui_if & p_Ui);
    ...
    template <class T>
      T * module_manager::create_module(const osm_diff_analyzer_if::module_configuration * p_conf)
      {
        ...
        if(l_iter == m_module_wrappers.end())
          {
            ...
            l_stream << "ERROR : Unknown module type \"" << p_conf->get_type() << "\"" ;
        ...
        if(l_result==NULL)
          {
            std::stringstream l_stream;
            l_stream << "ERROR : Creation of module \"" << p_conf->get_name() << "\" with type \"" << p_conf->get_type() << "\" has failed" ;
            throw quicky_exception::quicky_logic_exception(l_stream.str(),__LINE__,__FILE__);
          }
        return l_result;
      }
